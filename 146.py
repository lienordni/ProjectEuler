...
def check(n):
	...
	if(n%2==1 or n%3==0 or n%5!=0 or (n%7 not in [3,4]) or (n%11 in [3,8]) or (n%13 not in [1,3,4,9,10,12])):
		return False;

	for i in l:
		if lienprime(n*n+i)==False:
			return False

	pc=[]

	for i in range(n*n+1,n*n+28):
		if(lienprime(i)):
			pc+=[i]
